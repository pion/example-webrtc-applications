...
const (
	...
	videoFileName     = "output.h265"
	oggPageDuration   = time.Millisecond * 20
	h265FrameDuration = time.Millisecond * 33
)
...
func main() { //nolint
	...
	if haveVideoFile { // nolint
		...
		videoTrack, videoTrackErr := webrtc.NewTrackLocalStaticSample(webrtc.RTPCodecCapability{MimeType: webrtc.MimeTypeH265}, "video", "pion") // nolint
		...
		go func() {
			// Open a H265 file and start reading using our IVFReader
			file, h264Err := os.Open(videoFileName)
			if h264Err != nil {
				panic(h264Err)
			}

			h265, h264Err := h265reader.NewReader(file)
			if h264Err != nil {
				panic(h264Err)
			}
			...
			ticker := time.NewTicker(h265FrameDuration)
			for ; true; <-ticker.C {
				nal, h264Err := h265.NextNAL()
				...
				if h264Err = videoTrack.WriteSample(media.Sample{Data: nal.Data, Duration: h265FrameDuration}); h264Err != nil {
					panic(h264Err)
				}
			}
		}()
